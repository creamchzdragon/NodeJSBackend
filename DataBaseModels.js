...
class DbModel{
    ...
    getSingleByID(id,func){
        getQuery("SELECT * FROM "+this.tableName+" WHERE "+this.idField+" = "+id+" ;",
        function (err, result) {
            if (err) throw err;
    ...
    getListByFields(fields,orderby,orderMethod,func){
        ...
        getQuery("SELECT * FROM "+this.tableName+" WHERE "+eq+" "+" ORDER BY "+orderby.join(",")+";",
        function (err, result) {
           if (err) throw err;
                console.log("Result: " + result);
    ...
    getFieldById(id,fieldName,func){
        if(this.fields.includes(fieldName)){
            getQuery("SELECT "+fieldName+" FROM "+this.tableName+" WHERE "+this.idField+" = "+id+" ;",
            function (err, result) {
                if (err) throw err;
                    console.log("Result: " + result);
    ...
    saveToDb(func){
        ...
        getQuery("INSERT INTO "+this.tableName+" ( "+tableString+" ) VALUES "+tableValues+" ;",
        function (err, result) {
            if (err) throw err;
                console.log("Result: " + result);
                func();
